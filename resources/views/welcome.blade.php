...
<body class="antialiased">
    ...
    @if (session('alert'))
        <script>
            Swal.fire({
                icon: '{{ session('alert')['type'] }}', // success atau error
                title: '{{ session('alert')['type'] == 'success' ? 'Success' : 'Error' }}', // Judul alert
                text: '{{ session('alert')['message'] }}', // Pesan dari session
                showConfirmButton: false, // Hides the confirmation button
                timer: 5000, // The alert will close after 3 seconds (3000 ms)
            });
        </script>
    @endif
